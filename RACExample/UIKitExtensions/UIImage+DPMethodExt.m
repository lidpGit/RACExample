//
//  UIImage+DPMethodExt.m
//  UIKitExtensions
//
//  Created by lidp 

#import "UIImage+DPMethodExt.h"

@implementation UIImage (DPMethodExt)
...
+ (void)compressMessageImage:(UIImage *)image maxByte:(long long)maxByte callback:(void (^)(UIImage *, NSData *))callback{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        __block UIImage *resultImage = nil;
        __block NSData *data = nil;
        @autoreleasepool {
            data = UIImageJPEGRepresentation(image, 0.6);
            if (data.length > maxByte) {
                float scale = data.length / maxByte;
